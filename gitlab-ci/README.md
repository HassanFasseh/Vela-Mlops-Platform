...
| `GHCR_TOKEN` | A GitHub PAT with `write:packages` (and `read:packages`) scope - the same PAT used to recreate `ghcr-secret` by hand. It expires (~90 days); update this variable and re-run the pipeline when it does. |
